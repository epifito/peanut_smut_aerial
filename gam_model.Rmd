...
=======
(Manuscript's section `Results/3.2`)
>>>>>>> 17ed365 (Reknit gam_model)

```{r x-sq}
set.seed(3)
spore_type <- read_csv(here("data/spore_type.csv"),
...
The major spore type spread expected was the individual spore having a smaller spore size (18.8 µm); however, no statistical differences were recorded in spore type proportion at 0 m to 400 m downwind (p-value=`r spore_type$p.value`).
...
A script detailing the comparison of different models that were explored for the purposes of predicting the spores per cm^2^ is available for reference, [fit_incidence.R](fit_incidence.R "R script detailing several models that were evaluated for fitness with the data.").
This page only details the final best-fitting model that was selected for use.
=======
(Manuscript's section `Results/3.3`)
>>>>>>> 17ed365 (Reknit gam_model)
