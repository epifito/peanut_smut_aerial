...
m1 <- gam(
  spores_90min ~
    s(distance_m, wind_speed, by = time_slice, k = 55) +
    s(SDP, k = 6) +
    s(field, xy, bs = "re"),
  data = mod_dat,
  select = TRUE,
...
vis.gam(
  m1,
  theta = 50,
  view = c("wind_speed", "SDP"),
  ticktype = "detailed",
  main = "Wind speed (m/s) and severely damaged pods (SDP)"
)


a <- draw(smooth_estimates(m1, smooths(m1)[[1]])) + coord_cartesian()
b <- draw(smooth_estimates(m1, smooths(m1)[[2]]))
