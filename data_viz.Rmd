...
```{r}
library(tidyverse)
dat_spore_type <- read_csv("data/spore_type.csv")
```
...
dat_spore_type %>% 
  ...
  labs(y="Spore type frequency", x="Distance from harvesting field (m)", 
       fill="Nuclei per\npropagle unit")+
...
```{r}
dat_spore_size <- read_csv("data/spore_size.csv")
```
