...
ggplot(mod_dat, aes(y = spores_90min)) +
  geom_density()
```

Use dotplot over-layed with `stat_smooth()` to fit a line through the data.

```{r spore-dot-density-by-distance}
spore_dot_density_by_distance <- mod_dat %>%
  ggplot(aes(x = distance_m, y = spores_90min)) +
  geom_point() +
  geom_smooth(
    col = "grey50",
    method = "gam",
    formula = y ~ s(x, k = 3, bs = "fs")
  ) +
  labs(y = "Spores (n)",
  x = "Distance (m)")
...
Use boxplots to show the dispersal over distance.

```{r spore-boxplot-density-by-distance}
spore_box_density_by_distance <- mod_dat %>%
  ggplot(aes(x = as.factor(distance_m), y = spores_90min)) +
  geom_boxplot() +
  labs(y = "Spores (n)",
  x = "Distance (m)")
spore_box_density_by_distance
```
...
ggplot(all_data2,
       aes(x = as.factor(distance_m), y = spores_90min,
           fill = time_slice)) +
...
```{r spore-windspeed}
ggplot(data = mod_dat, aes(x = wind_speed, y = spores_90min)) +
